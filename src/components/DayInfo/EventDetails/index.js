...
const EventDetails = () => {
  const [{ eventDetailsOpen, selectedEvent, selectedDay }, dispatch] = useStateValue()
  ...
  let reverseFormat = (str) => {
    ...
    let data = str.split('-').map((e, i) => (i === 1 ? parseInt(e - 1) : parseInt(e)))
    ...
    <S.Container shown={eventDetailsOpen}>
      ...
      <S.Form onSubmit={(e) => handleFormSubmit(e)}>
        ...
        <S.Input
          ...
          id="date"
          required={true}
          onChange={({ target: { value } }) => handleChange('date', reverseFormat(value))}
        />
        ...
        <S.Input
          ...
          id="date"
          required={true}
          onChange={({ target: { value } }) => handleChange('hours', updateHours('start', value))}
        />
        <S.Time htmlFor="end">
          <span>End:</span>
        </S.Time>
        ...
        <S.Input
          ...
          id="date"
          required={true}
          onChange={({ target: { value } }) => handleChange('hours', updateHours('end', value))}
        />
        <S.Actions>
          <S.Button
            type="button"
